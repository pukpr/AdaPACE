package Abk is
   ...
   -- the difference in elevation between the delivery vehicle and the target.
   -- Input angle (radians) and velocity (m/s) and vertical_distance (m)
   -- Vertical_Distance should be the difference in elevation between delivery vehicle and the target,
   -- so if target is higher than delivery vehicle the value should be negative.
   ...
   -- Note: Assumes delivery vehicle and target are at same elevation
   ...
   -- returns the horizontal distance difference between the target's location
