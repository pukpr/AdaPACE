...
package body Post.A is
   ...
   type Extra is new Pace.Msg with null record;
   procedure Input (Obj : in Extra);

   procedure Input (Obj : in Fourth) is
   begin
      Pace.Log.Wait (1.0);
      --Pace.Log.Trace (Obj);
      Pace.Dispatching.Input (Post.B.Start'(Pace.Msg with null record));
      Pace.Surrogates.Input (Extra'(Pace.Msg with null record));
   end Input;

   procedure Input (Obj : in Extra) is
   begin
      Pace.Log.Wait (4.0);
      --Pace.Log.Trace (Obj);
   end Input;
