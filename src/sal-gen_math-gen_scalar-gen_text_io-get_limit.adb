--  Abstract :
--
--  Raise Invalid_Limit for invalid limits
--
...
is
   ...
   Temp_Item : Limit_Type;
begin
   Check (File, "(");
   if Named_Association_Record then
      Check (File, "Low => ");
   end if;
   Math_Text_Io.Get (File, Temp_Item.Low);
   Check (File, ",");
   Skip_Whitespace (File);
   if Named_Association_Record then
      Check (File, "High => ");
   end if;
   Math_Text_Io.Get (File, Temp_Item.High);
   Check (File, ")");

   Item := To_Limit (Low => Temp_Item.Low, High => Temp_Item.High);
end Get_Limit;
