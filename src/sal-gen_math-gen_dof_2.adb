...
package body Sal.Gen_Math.Gen_Dof_2 is
   ...
   function Mag (Right : in Point_Type) return Real_Type is
   begin
   ...
   function Value (Image : in String) return Point_Type is
   ...
   begin
      return
        (Real_Type'Value (Image (Right_Paren + 1 .. Comma - 1)),
   ...
   function Image (Item : in Rectangle_Type) return String is
   begin
      return "(" &
             Image (Item.Left_Bottom) &
             ", " &
             Image (Item.Right_Top) &
             ")";
   end Image;

   function Value (Image : in String) return Rectangle_Type is
      First_Point  : constant Natural := Ada.Strings.Fixed.Index (Image, "(");
      Second_Point : constant Natural :=
         Ada.Strings.Fixed.Index (Image, "),");
   begin
      return
        (Value (Image (First_Point + 1 .. Second_Point + 1)),
         Value (Image (Second_Point + 2 .. Image'Last)));
   end Value;

   function Size (Item : in Rectangle_Type) return Point_Type is
   begin
      return (Item.Right_Top - Item.Left_Bottom);
   end Size;

   function "+" (Left, Right : in Rectangle_Type) return Rectangle_Type is
   begin
      return
   ...
   function Clip (Inner, Outer : in Rectangle_Type) return Rectangle_Type is
   begin
      return
        (Left_Bottom => (X =>
                            Real_Type'Min
   ...
   is
   begin
      return Point.X >= Outer.Left_Bottom.X and
   ...
   is
   begin
      return Inside (Inner.Left_Bottom, Outer) and
             Inside (Inner.Right_Top, Outer);
   end Inside;
   ...
   begin
      return
        (Left_Bottom => (X => Center.X - Size.X / 2.0,
                         Y => Center.Y - Size.Y / 2.0),
         Right_Top   => (X => Center.X + Size.X / 2.0,
                         Y => Center.Y + Size.Y / 2.0));
   end Make_Rectangle;
   ...
   function Center
     (Inner_Size : in Point_Type;
      ...
      return       Point_Type
   is
      Outer_Center : constant Point_Type :=
         (Outer.Left_Bottom + Outer.Right_Top) / 2.0;
   begin
