...
is
   ...
   UDB : Buffer_Pointer        :=
      new Buffer (1 .. Image_Size (Four, Bit_Depth, X, Y, Interlace));
   UDP : Stream_Element_Offset := UDB'First;

begin
   Start_File (Filename, X, Y, Four, Bit_Depth, Interlace, F);
   ...
   -- parameters are of type Dimension, for which the lower bound is 1.
      ...
      procedure Write_Image (X, Y : in Dimension; Pass : Pass_Number := 1) is
         Bps               : constant Stream_Element_Count   :=
            Bytes_per_Scanline (Four, Bit_Depth, X);
            ...
            for Col in  0 .. Coordinate (X - 1) loop
               ...
               begin
                  ...
                  case Bit_Depth is
                     when Eight =>
                        Scanline (P .. P + 1) :=
                          Stream_Element (Grey_Sample (I, R, C)) &
                          Stream_Element (Alpha_Sample (I, R, C));
                     when Sixteen =>
                        Scanline (P .. P + 3) :=
                          To_Buffer_2 (Unsigned_16 (Grey_Sample (I, R, C))) &
                          To_Buffer_2 (Unsigned_16 (Alpha_Sample (I, R, C)));
                  end case;
               end;
            end loop;
