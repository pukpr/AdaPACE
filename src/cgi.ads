...
package Cgi is
   ...
   -- This package automatically loads information from CGI on program
   --start-up.
   -- It loads information sent from "Get" or "Post" methods and automatically
   -- splits the data into a set of variables that can be accessed by position
   --or
   ...
   -- These routines find the Index'th value of that key (normally the first
   --one).
   function Value
   ...
   function Value_Of_Line (Key : String; Position : Positive) return String;
   -- Given a Key which has a Value that may have multiple lines,
   ...
   -- Even if the data appears to have come from the user, it should be
   --filtered;
