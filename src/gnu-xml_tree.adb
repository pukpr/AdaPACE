...
package body Gnu.Xml_Tree is
   ...
   begin
      ...
      if N.Child /= null then
         Put_Line (">");
         Print (N.Child, Indent + 2, Display);
         P := N.Child.Next;

         while P /= null loop
            Print (P, Indent + 2, Display);
            P := P.Next;
         end loop;
...
------------------------------------------------------------------------------
-- $id: gnu-xml_tree.adb,v 1.6 07/31/2003 15:33:14 ludwiglj Exp $
------------------------------------------------------------------------------
end Gnu.Xml_Tree;
