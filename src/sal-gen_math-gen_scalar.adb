...
package body Sal.Gen_Math.Gen_Scalar is
   ...
   function Modulo (Dividend, Divisor : in Real_Type) return Real_Type is
   ...
   function To_Limit (Low, High : in Real_Type) return Limit_Type is
   begin
      if Low >= High then
         raise Invalid_Limit;
      else
   ...
   function High (Item : in Limit_Type) return Real_Type is
   begin
      return Item.High;
   end High;

   function Low (Item : in Limit_Type) return Real_Type is
   begin
      return Item.Low;
   end Low;

   function "and" (Left, Right : in Limit_Type) return Limit_Type is
   begin
      return To_Limit
               (Real_Type'Max (Left.Low, Right.Low),
                Real_Type'Min (Left.High, Right.High));
   end "and";
   ...
   function First_Order_Trig return Real_Type is
   begin
      return Elementary.Sqrt (Real_Type'Model_Epsilon);
   end First_Order_Trig;

   function Sin_Cos (Angle : in Real_Type) return Trig_Pair_Type is
   begin
      return (Elementary.Sin (Angle), Elementary.Cos (Angle));
   end Sin_Cos;

   function Atan2 (Trig : in Trig_Pair_Type) return Real_Type is
   begin
      return Elementary.Arctan (Trig.Sin, Trig.Cos);
   end Atan2;

   function Sin (Trig : in Trig_Pair_Type) return Real_Type is
   begin
      return Trig.Sin;
   end Sin;

   function Cos (Trig : in Trig_Pair_Type) return Real_Type is
   begin
      return Trig.Cos;
   end Cos;

   function To_Trig_Pair (Sin, Cos : in Real_Type) return Trig_Pair_Type is
   ...
   function Unchecked_Trig_Pair
     (Sin, Cos : in Real_Type)
      return     Trig_Pair_Type
   is
   begin
      return (Sin, Cos);
   ...
   function "+"
     (Left  : in Trig_Pair_Type;
      Right : in Trig_Pair_Type)
      return  Trig_Pair_Type
   is
   begin
      return
        (Left.Sin * Right.Cos + Left.Cos * Right.Sin,
         Left.Cos * Right.Cos - Left.Sin * Right.Sin);
   end "+";
   ...
   function Half_Trig (Trig : in Trig_Pair_Type) return Trig_Pair_Type is
   ...
   function Double_Trig (Trig : in Trig_Pair_Type) return Trig_Pair_Type is
   begin
      return
        (2.0 * Trig.Sin * Trig.Cos,
         Trig.Cos * Trig.Cos - Trig.Sin * Trig.Sin);
   end Double_Trig;
