...
package Pace.Xml_Tree is
   ...
   procedure Null_Print (Text : in String);
   
   procedure Print (Root : in out Tree;  -- out stores the string, so can get it.
                    Indent : in Natural := 0;
                    Display : in Print_Callback := null);
   --  Simple print procedure. Print the whole tree starting with N.
   function Get_Tree (Root : Tree) return String;
   --  Simple print procedure. Print the whole tree starting with N.
...
private
   ...
   type Tree is new Ada.Finalization.Limited_Controlled with
      record
         N : Node_Ptr;
         S : Unbounded_String; -- used to print display
      end record;
...
------------------------------------------------------------------------------
-- $Id: pace-xml_tree.ads,v 1.2 2006/04/10 15:51:41 pukitepa Exp $
------------------------------------------------------------------------------
end Pace.Xml_Tree;
