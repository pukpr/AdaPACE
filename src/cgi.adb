...
package body Cgi is
   -- This package is an Ada 95 interface to the "Common Gateway Interface"
   --(CGI).
   ...
   Unescaped_Url : constant Character_Set     :=
   ...
   begin
      ...
      while I <= Last_Possible_Position loop
         if Element (Data, I) = '%'
           and then Is_Hexadecimal_Digit (Element (Data, I + 1))
           and then Is_Hexadecimal_Digit (Element (Data, I + 2))
         then
            Replace_Element
              (Data,
               I,
   ...
   begin
      ...
      while I <= Last_Position loop
         ...
         if Translate_Plus and Current_Character = '+' then
            ...
            Replacer (2) :=
               To_Hex_Char (Character'Pos (Current_Character) / 16);
            Replacer (3) :=
               To_Hex_Char (Character'Pos (Current_Character) mod 16);
   ...
   begin
      while I <= Current_Length loop
         ...
         if Current_Character = '&' then
            ...
            -- &quot; correctly.  However, we HAVE to do this, because
            --otherwise
            -- attribute values will incorrectly terminate in odd places.
            -- It _is_ in the standard, and current versions of Netscape
            --Navigator,
   ...
   function Url_Encode (Data : in String;
                         Translate_Plus : Boolean := False) return String is
      Destination : Unbounded_String := To_Unbounded_String (Data);
   begin
   ...
   function Get_Environment (Variable : String) return String is
      ...
      Result               : String    :=
         Value_Without_Exception (Result_Ptr);
   begin
   ...
   begin
      if Cgi_Data = null then
         return 0;
      else
         return Cgi_Data.all'Length;
      end if;
   end Argument_Count;
   ...
   begin
      for I in 1 .. Argument_Count loop
         if Cgi_Data.all (I).Key = Key
           and then Cgi_Data.all (I).Value = Value
         then
            return True;
         end if;
      end loop;
   ...
   begin
      return "http://" &
             Get_Environment ("SERVER_NAME") &
             Get_Environment ("SCRIPT_NAME");
   end My_Url;

   procedure Put_Cgi_Header
   ...
   begin
      Put_Line
        ("<h" & Image (Level) & ">" & Title & "</h" & Image (Level) & ">");
   ...
   function Next_Crlf (S : in String; N : in Natural) return Natural
   ...
   function Line (Value : in String; Position : in Positive) return String
   ...
   begin
      Cgi_Data.all (Key_Number).Key   :=
         To_Unbounded_String (Slice (Datum, 1, Last));
      Cgi_Data.all (Key_Number).Value :=
   ...
   begin
      return To_String
               (Cookie_Value (To_Unbounded_String (Key), Index, Required));
   end Cookie_Value;
...
--      Temp_Ptr          : Access_Key_Value_Sequence;
   ...
   begin -- read_cookie
      ...
      if Raw_Data /= "" then
         ...
         Number_Of_Cookies :=
           Ada.Strings.Unbounded.Count (Raw_Data, Semicolon) + 1;
   ...
   begin
      ...
      Translate (Raw_Data, Mapping => Plus_To_Space); -- Convert "+"s to
                                                      --spaces.
      ...
      if Length (Raw_Data) > 0 then
         ...
         Cgi_Data :=
           new Key_Value_Sequence (
            1 .. Ada.Strings.Unbounded.Count (Raw_Data, Ampersands) + 1);
