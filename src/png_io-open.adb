...
begin
   ...
   if F /= null then
      Raise_Exception
        (Call_Error'Identity,
         "PNG File " & Filename & " is already open.");
   end if;
   ...
   Read_Chunks : declare
      ...
      function Read_Chunk return Buffer_Pointer is
         ...
         -- as well as the data, so we must add 4 to the length read from the
         --file.

         Chunk_Length : constant Unsigned_32 := Read_Unsigned_32;

         BP : Buffer_Pointer :=
            new Buffer (1 .. Stream_Element_Count (Chunk_Length + 4));
         B  : Buffer renames BP.all;
      begin

         -- Read the chunk into the buffer. Note that if the chunk length was
         --zero,
         ...
         -- the whole content of the buffer B. The CRC at the end of the chunk
         --is
         -- the 1's complement of the CRC computed over the chunk type and
         --data.

         declare
            NChunk_CRC : constant ZLib.Unsigned_32 :=
               ZLib.Unsigned_32 (Read_Unsigned_32);
   ...
   begin
      ...
      -- Read the IHDR chunk (which must come next, immediately after the
      --signature).
      ...
      begin
         ...
         if B'Length /= 17
           or else To_Unsigned_32 (B (1 .. 4)) /= IHDR
         then
            Deallocate (BP);
            raise Format_Error;
         end if;
      ...
      -- Check the colour type and the bit depth together since these are
      --inter-related.
...
"Invalid combination of colour type and bit depth in IHDR chunk.");
      ...
      begin
         if T = 0 then
            ...
            Raise_Exception
              (Format_Error'Identity,
               "Invalid colour type in IHDR chunk.");
         end if;
      end;
      ...
      declare
         ...
         -- We verify the first two bytes of the IDAT stream (and of any zTXt
         --streams too).
         -- Since IDAT chunks of length 1 (and even 0!) are legal, we can't
         --guarantee that
         -- the first two bytes will be found in the first IDAT chunk (or even
         --in consecutive
         -- IDAT chunks, since zero length IDAT chunks could occur). Therefore
         --we may have to
         -- save the value of the first byte and check the two bytes only when
         --we have read
         -- the second.

         IDAT_1 : Stream_Element;   -- To store the first two bytes of the
                                    --IDAT stream.
         IDAT_V : Boolean := False; -- Set True when we have seen the second
                                    --byte and tested.

         procedure Validate_Zlib_Stream
           (CMG, FLG : in Stream_Element;
            Message  : in String)
         is
         begin
      ...
      begin
         ...
         -- Allocate a buffer for the uncompressed image data in the IDAT
         --chunks.
         -- The size of this buffer is exactly that needed for the
         --uncompressed pixel
         -- data, plus one byte. The reason for this extra byte is that
         --without it, a
         -- zero length out buffer may get passed to Zlib on the last
         --Translate call.
         -- It is possible for up to 4 bytes to be passed into Zlib at the end
         --of a
         -- compressed stream, with no output (the last 4 bytes are the
         --Adler32 checksum).
         -- Zlib_Ada V1.2 checks for a zero length buffer, so we have to make
         --sure we
         -- always give it at least one byte, even if this byte will never get
         --used.
         ...
         For_each_chunk : while Previous_Chunk_Type /= IEND loop  -- Read all
                                                                  --the chunks,
            declare                                               -- including
                                                                  --IEND.
               BP : Buffer_Pointer := Read_Chunk;
               ...
               Chunk_Type   : constant Unsigned_32          :=
                  To_Unsigned_32 (B (1 .. 4));
            ...
            begin
               case Chunk_Type is
                  when PLTE =>
                     ...
                     -- Allocate a palette and copy the colour palette data to
                     --it
                     -- from the chunk buffer.

                     F.Palette :=
                       new Colour_Palette (Unsigned_8 (Chunk_Length / 3 - 1));
                     for X in  F.Palette.R'Range loop
                        declare
                           Y : constant Stream_Element_Offset :=
                              3 * Stream_Element_Offset (X);
                        begin
                  ...
                  when IDAT =>
                     ...
                     Decompress : declare
                        ...
                        use ZLib;
                     begin
                        if Chunk_Length /= 0 then  -- If this is a zero length
                                                   --IDAT, skip.
                           if not IDAT_Flag then

                              -- This is the first IDAT chunk (or the first of
                              --non-zero length!).

                              Inflate_Init (Z);    -- Initialise the Zlib
                                                   --decompressor.

                              IDAT_Flag := True;   -- Note that we have seen
                                                   --the first IDAT, and
                                                   -- therefore that the Zlib
                                                   --decompressor has been
                                                   -- intialised (and must be
                                                   --closed if we exit due
                                                   -- to an exception).

                              if Chunk_Length > 1 then

                                 -- We can check the first two bytes now.
                                 ...
                                 Validate_Zlib_Stream
                                   (B (5),
                                    B (6),
                                    "Invalid Zlib header in first IDAT chunk.")
;
                                 IDAT_V := True;
                              else

                                 -- We have to store the first byte, and check
                                 --the second one later when
                                 -- we get another chunk with non-zero length.

                                 IDAT_1 := B (5);
                              end if;

                              -- This is the first IDAT chunk so we can use
                              --the whole of the
                              -- uncompressed data buffer (we may need to, if
                              --this is the only
                              -- IDAT chunk). If there are other IDAT chunks,
                              --then this call
                              -- to Zlib will not fill the uncompressed data
                              --buffer. If this
                              -- chunk has length one, we will pass only the
                              --first header byte
                              -- into Zlib (allowed).
                              ...
                              -- This is not the first (non-zero length) IDAT
                              --chunk, so we need to
                              -- get Zlib to store the decompressed data after
                              --any data that is already
                              -- in the uncompressed data buffer. We also have
                              --to check the first
                              ...
                              if not IDAT_V then
                                 Validate_Zlib_Stream
                                   (IDAT_1,
                                    B (5),
                                    "Invalid Zlib header in IDAT chunks.");
                                 IDAT_V := True;
                              end if;
                              ...
                              Translate
                                (Z,
                                 B (5 .. B'Last),
                                 I,
                                 U (Total_Out (Z) + 1 .. U'Last),
                                 O,
                                 No_Flush);
                           end if;

                           pragma Assert (I = B'Last);   -- There doesn't seem
                                                         --to be any similar
                                                         -- check we can do on
                                                         --the value of O.
                        end if;
                  ...
                  when IEND =>
                     ...
                     Close (F.Handle); -- If there is any more data in the
                                       --file, we ignore it.
                     if not IDAT_Flag then
                        Raise_Exception
                          (Format_Error'Identity,
                           "No IDAT chunks in file.");
                     end if;
                  ...
                  when cHRM =>
                     ...
                     if IDAT_Flag or PLTE_Flag then
                        Raise_Exception
                          (Format_Error'Identity,
                           "cHRM chunk after IDAT/PLTE.");
                     elsif F.Chroma then
                        Raise_Exception
                          (Format_Error'Identity,
                           "Multiple cHRM chunks.");
                     end if;
                     F.Chroma  := True;
                  ...
                  when gAMA =>
                     ...
                     if IDAT_Flag or PLTE_Flag then
                        Raise_Exception
                          (Format_Error'Identity,
                           "gAMA chunk after IDAT/PLTE.");
                     elsif F.Gamma then
                        Raise_Exception
                          (Format_Error'Identity,
                           "Multiple gAMA chunks.");
                     end if;
                     F.Gamma       := True;
                  ...
                  when sRGB =>
                     ...
                     if IDAT_Flag or PLTE_Flag then
                        Raise_Exception
                          (Format_Error'Identity,
                           "sRGB chunk after IDAT/PLTE.");
                     elsif F.SRGB then
                        Raise_Exception
                          (Format_Error'Identity,
                           "Multiple sRGB chunks.");
                     end if;
                     F.SRGB := True;
                     declare
                     ...
                     begin
                        if B5 not  in
                              Rendering_Intent'Pos (Rendering_Intent'First) ..
                              Rendering_Intent'Pos (Rendering_Intent'Last)
                        then
                           Raise_Exception
                             (Format_Error'Identity,
                              "sRGB chunk has bad value.");
                        end if;
                  ...
                  when pHYs =>
                     ...
                     if IDAT_Flag then
                        Raise_Exception
                          (Format_Error'Identity,
                           "pHYs chunk after IDAT.");
                     elsif F.Physical then
                        Raise_Exception
                          (Format_Error'Identity,
                           "Multiple pHYs chunks.");
                     end if;
                     F.Physical  := True;
                  ...
                  when tEXt | zTXt =>
                     ...
                     F.Text_Strings :=
                       new Text_Item'(null, null, F.Text_Strings);

                     -- Read the keyword string, verifying that the characters
                     --in
                     -- the string are allowed characters. (PNG Specification
                     --1.2
                     -- section 4.2.3).

                     declare
                        K : String (1 .. 80);   -- The keyword string is
                                                --always less than
                                                -- 80 characters in length,
                                                --but we need to
                                                -- allow space for a NUL at
                                                --the end.
                        use Ada.Characters.Latin_1;
                     begin
                        for I in  K'Range loop  -- Read the chunk data up to
                                                --the null.
                           K (I) :=
                              Character'Val
                                (Natural (B (Stream_Element_Offset (I) + 4)));
                           if K (I) = NUL then

                              -- We have reached the end of the keyword
                              --string. Check
                              -- that it has no leading or trailing spaces, or
                              --consecutive
                              -- spaces.

                              if I = 1       -- The keyword must be at least
                                             --one character.
                                or else K (1) = Space -- Leading spaces are
                                                      --not permitted.
                                or else K (I - 1) = Space   -- Trailing spaces
                                                            --ditto.
                              then
                                 Raise_Exception
                                   (Format_Error'Identity,
"Illegal use of spaces in text chunk keyword.");
                              ...
                              F.Text_Strings.Keyword :=
                                new String'(K (1 .. I - 1));
                              exit;
                           end if;

                           -- The character just read was not a NUL, check its
                           --legality.
...
"Illegal non-break space character in text chunk keyword.");
                           elsif not Ada.Characters.Handling.Is_Graphic
                                       (K (I))
                           then
                              Raise_Exception
                                (Format_Error'Identity,
"Illegal non-graphic character in text chunk keyword.");
                        ...
                        -- The rest of the chunk is either compressed text or
                        --plain text.
                        -- If it is plain text, all we need to do is copy it
                        --to the
                        -- descriptor. If it is compressed we need to
                        --decompressed it and
                        -- then copy it to the descriptor.

                        declare
                           T : Buffer renames B (
                              F.Text_Strings.Keyword'Length + 6 .. B'Last);
                        begin
...
"Unknown compression method in zTXt chunk.");
                              ...
                              -- Decompress the text. We don't know the size
                              --of the
                              -- decompressed text, and assuming that zTXt
                              --chunks will not
                              -- contain huge amounts of text, we adopt the
                              --kludge of
                              -- allocating a buffer of a fixed multiple of
                              --the compressed
                              -- buffer size and hope for the best.
                              -- Pity the PNG design team didn't think to
                              --include the size
                              -- of the text in the chunk! It would have been
                              --easily done
                              -- with a few extra bytes after the compression
                              --method.

                              declare
                                 DP : Buffer_Pointer :=
                                    new Buffer (1 .. T'Length * 10);
                              ...
                              begin
                                 ...
                                 -- Copy the text out of the D buffer into the
                                 --text item.

                                 F.Text_Strings.Text_String :=
                                   new String (1 .. Natural (O));
                                 for I in  F.Text_Strings.Text_String'Range
                                 loop
                                    F.Text_Strings.Text_String (I) :=
                                       Character'Val
                                         (D (Stream_Element_Offset (I)));
                                 end loop;
                              ...
                              -- Copy the text out of the T buffer into the
                              --text item.

                              F.Text_Strings.Text_String :=
                                new String (1 .. T'Length);
                  ...
                  when others =>
                     if Ancillary (Chunk_Type) then   -- add it to the linked
                                                      --list in the descriptor.
                        F.Number_of_Chunks := F.Number_of_Chunks + 1;
                        declare
                           Temp : constant Chunk_List :=
                              new Chunk_List_Element (Chunk_Length);
                           C    : Chunk renames Temp.Chnk;
                        begin
                           C.Name := To_Chunk_Name (Chunk_Type);
                           for X in  C.Data'Range loop
                              C.Data (X) :=
                                B (Stream_Element_Offset (X) + 4);
                           end loop;

                           if Known_Chunk (Chunk_Type) then

                              -- We know where this chunk should occur.  We
                              --can verify that the
                              -- position in the file is legal, AND set the
                              --Where field to what
                              -- it should be.
                              ...
                              if Before_PLTE (Chunk_Type) and PLTE_Flag then
                                 Raise_Exception
                                   (Format_Error'Identity,
                                    To_Chunk_Name (Chunk_Type) &
                                    " found after PLTE chunk.");
                              elsif Before_IDAT (Chunk_Type) and
                                    IDAT_Flag
                              then
                                 Raise_Exception
                                   (Format_Error'Identity,
                                    To_Chunk_Name (Chunk_Type) &
                                    " found after IDAT chunk.");
                              end if;

                           else

                              -- We don't know where this chunk should occur,
                              --so we cannot
                              -- check the legality of its positioning in the
                              --file. We have
                              -- to deduce the Where field from the position
                              --actually found
                              -- in the file, although we may not yet have
                              --seen the PLTE if
                              -- present. This isn't totally satisfactory, but
                              --it is unlikely
                              -- to cause problems in practice, since it is
                              --unlikely that an
                              -- unknown ancillary chunk must precede PLTE.

                              if IDAT_Flag then
                                 C.Where := Anywhere;    -- The chunk was
                                                         --found after IDAT.
                              else
                                 C.Where := Before_IDAT; -- The chunk was
                                                         --found before IDAT.
                              end if;
                           end if;
         ...
         if IDAT_Flag then
            ...
            -- We have seen one or more IDAT chunks and therefore the Zlib
            --filter will have been
            -- initialised/opened, and we must close it. Check also that all
            --the IDAT data has
            -- come out of ZLib. There is a spare byte at the end of the
            --uncompressed data buffer
            -- which we have to allow for (see above for a description of why
            --it is there).

            if ZLib.Stream_End (Z) and
               ZLib.Total_Out (Z) = F.Uncompressed_Data.all'Length - 1
            then
               ZLib.Close (Z);
               IDAT_Flag := False;  -- This indicates that the Zlib filter has
                                    --been closed.
            else
               Raise_Exception
                 (Format_Error'Identity,
   ...
   Defilter : declare
      ...
      Bpp : constant Natural :=
         Bytes_per_Pixel (Colour_Type (F), Bit_Depth (F));
      ...
      procedure Pass (N_Lines, N_Pixels : in Natural) is
         ...
         W : constant Natural :=
            (N_Pixels * Bits_per_Pixel (Colour_Type (F), Bit_Depth (F)) +
             7) /
            8;
      begin
         for Y in  1 .. N_Lines loop   -- We have to allow for the possibility
                                       --that a pass
            -- has zero columns or rows as described in Section
            -- 2.6 of the PNG Specification. If N_Lines is zero this
            -- loop will execute zero times and Pass will do nothing.
            exit when W = 0;     -- If W is zero we need to skip the code in
                                 --the loop.
            declare
            -- P is the index of the first byte of the previous scanline,
            --except when Y = 1.
               P  : constant Stream_Element_Offset :=
                  Stream_Element_Offset'Max
                    (1,
                     Pointer - Stream_Element_Offset (W + 1));
      ...
      case Interlaced (F) is
         ...
         when True =>
            ...
            for P in  Pass_Number loop
               F.Interlace_Offsets (P) := Pointer; -- Remember for use in
                                                   --reading
                                                   -- the pixels later.
               Pass
                 (Sub_Image_Height (Height (F), P),
                  Sub_Image_Width (Width (F), P));
            end loop;
      end case;

      if Pointer /= F.Uncompressed_Data'Length then
         Raise_Exception
           (Format_Error'Identity,
...
exception
   when others =>
      Close (F); -- This also deallocates the palette etc.
      raise;
end Open;
