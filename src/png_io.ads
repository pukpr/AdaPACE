...
---------------------------------------------------------------------
-- Date:   26   August 1999                                        --
-- Edited: 15 November 1999 to add comment about row numbering.    --
--          1 November 2000 to add sRGB read functions and support --
--                          for writing ancillary chunks: changed  --
--                          spec version to 2.                     --
--          9    March 2001 to add additional sRGB functions: spec --
--                          version 3 and default for Gamma_Chunk. --
--         15      May 2002 to add the function Sample_Depth.      --
--         30      May 2002 to add the function Interlaced.        --
--          7     June 2002 to amend comments about Pixel_Value in --
--                          the case of Type 3 images.             --
--         23    April 2003 removed incorrect comment about the    --
--                          Interlace parameters of the output     --
--                          procedures.                            --
--          9  January 2004 to insert a reference to the ISO std.  --
--         10  January 2004 to add the function Zlib_Version.      --
--          7     July 2004 to use Ada.Streams for passing chunks. --
---------------------------------------------------------------------
---------------------------------------------------------------------
...
--                                                                 --
-- PNG_IO is intended to provide medium-level input and output     --
-- only and is intended to be used as a foundation to develop      --
-- higher level PNG file I/O packages for specific applications.   --
---------------------------------------------------------------------
---------------------------------------------------------------------

with Ada.Streams, Ada.IO_Exceptions; use Ada.Streams;

package PNG_IO is

   function Version return String; -- The package version, e.g. "1.0".
   ...
   -- top and left of the image. Row 0 is the topmost row of the image, column
   --0
   -- is the leftmost column. This is conventional for graphics and is often
   --the
   ...
   -- A PNG file can contain five types of image with various bit depths
   --allowed
   -- for each type. Two enumerated types indicate the type of image in a file,
   -- and the number of bits per sample.

   type Depth is (One, Two, Four, Eight, Sixteen); -- The number of bits per
                                                   --sample.
   ...
   type Colour_Type_Code is (Zero, Two, Three, Four, Six);  -- The PNG Colour
                                                            --Type.

   function Colour_Type (F : PNG_File) return Colour_Type_Code;

   -- For convenience, the following tables indicate whether a file of a
   --particular
   -- colour type has alpha information, and/or contains a colour image
   --(including
   -- a palette colour image).

   Alpha  : constant array (Colour_Type_Code) of Boolean :=
     (False,
      False,
      False,
      True,
      True);
   Colour : constant array (Colour_Type_Code) of Boolean :=
     (False,
      True,
      True,
      False,
   ...
   function Palette (F : PNG_File) return Boolean; -- True if the file has a
                                                   --palette.
   function Palette_Size (F : PNG_File) return Positive;

   -- Palette indices run from 0 .. Palette_Size - 1;

   function Palette_R_Value (F : PNG_File; Index : Natural) return Natural;
   function Palette_G_Value (F : PNG_File; Index : Natural) return Natural;
   function Palette_B_Value (F : PNG_File; Index : Natural) return Natural;
   ...
   function Text_Keyword (F : PNG_File; N : Positive) return String;
   function Text_String (F : PNG_File; N : Positive) return String;

   ----------------------------------------------------------------------------
   ----------

   -- Access to unrecognised or unsupported ancillary chunks found in a PNG
   --file on
   -- read. The name of, and raw data within, these chunks is not understood
   --by PNG_IO,
   -- although it is verified that the chunks are ancillary.  All such chunks
   -- encountered in a PNG file on read are available through the following
   --functions.
   -- They may be passed back into PNG_IO for writing to an output file but
   --the user
   -- code should check that the chunk is safe-to-copy if it does not
   --understand the
   -- chunk.

   type Chunk (<>) is private;

   function Ancillary_Chunk_Count (F : PNG_File) return Natural;  -- There may
                                                                  --be none.
   function Ancillary_Chunk (F : PNG_File; N : Positive) return Chunk;

   function Safe_to_Copy (C : Chunk) return Boolean;

   subtype Chunk_Name is String (1 .. 4);

   -- Since PNG_IO does not understand the data within the chunk, it can only
   --be passed
   -- in 'raw' form. Prior to version 4, the data was passed as an array of
   --Unsigned_8
   -- values, but from version 4 onwards this was changed to use
   --Ada.Streams.Stream_Elements
   ...
   -- Ancillary chunks may be written to files. The mechanism provided is very
   --general
   -- and is designed to be readily extensible and compatible with that
   --provided for
   -- handling chunks read from a file (see immediately above).  Each generic
   --write
   -- procedure (see below) takes a list of chunks as a parameter. The list
   --may include
   -- those created by functions in this package (e.g. gamma) and/or chunks
   --created by
   -- child packages using the primitives provided here (the full type Chunk
   --is visible
   -- to children of this package to allow extension of the chunk handling
   --defined here)
   -- and/or chunks read from a PNG file.

   type Chunk_List is private;               -- NB The type is dynamically
                                             --allocated.
   ...
   function "&" (Left : Chunk; Right : Chunk_List) return Chunk_List;
   function "&" (Left : Chunk_List; Right : Chunk) return Chunk_List;

   type Chunk_Position is (
      Before_PLTE, -- and therefore before IDAT too,
      Before_IDAT, -- but after PLTE if present,
      Anywhere);   -- interpreted as after IDAT.

   -- PNG_IO has some built-in knowledge of ancillary chunks (those defined in
   --the
   -- PNG Specification V1.2). To allow the user to look up the position of
   --such
   -- chunks the following two functions are provided. The Position function
   --raises
   ...
   -- Function for constructing arbitrary chunks. The chunk length is inferred
   --from
   ...
   function Gamma_Chunk (Gamma : Natural := 100_000) return Chunk;
   ...
   function Chromaticity_Chunk
     (White_Point, Red_Primary, Green_Primary, Blue_Primary : Pair)
      return                                                  Chunk;
   function Physical_Chunk (Value : Pair; Metre : Boolean) return Chunk;
   ...
   -- Procedures for writing PNG files. There is one procedure for each colour
   --type.
   -- The pixel values are accessed using generic function parameter(s)
   --supplied by
   -- the user. This allows the procedures to be independent of the user's
   --pixel
   -- representations. Instantiations of these procedures will write a
   --particular
   ...
   -- functions which access the pixel values to have some form of image
   --handle.
   ...
   -- pointer/handle or similar, including a PNG_File handle as in the
   --functions
   -- above which implement the corresponding operation for reading PNG files.
   -- Pixel/sample values are passed as integers and the range of the type
   --supplied
   -- must be consistent with the bit depth given when the procedure was
   --instantiated
   -- (that is, having at least the range demanded by the bit depth). The
   --values
   -- supplied must not exceed the largest value allowed by the bit depth.

   -- The Ancillary parameter was added at the end of the list of parameters
   --for
   -- backward compatibility with code written for previous versions of PNG_IO.
   -- This means that if a value is supplied for the Ancillary parameter, a
   --value
...
private
   ...
   -- It is assumed in this specification that Natural has a large enough range
   -- to be able to pass 32-bit values such as image dimensions from PNG files.
   ...
   -- The type Stream_Element is defined in Ada.Streams as "mod implementation
   --defined".
   -- The most likely implementation is a byte, i.e. mod 2**8, on all modern
   --systems,
   -- but it is not guaranteed that this will be so. If Stream_Element is not
   --a byte,
   -- PNG_IO will not compile and work without modification (neither will
   --Zlib_Ada).
