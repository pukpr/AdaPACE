...
package body Sal.Gen_Math.Gen_Stats is
   ...
   procedure Accumulate (Stats : in out Stats_Type; Value : in Real_Type) is
   begin
      Stats :=
        (Count       => Stats.Count + 1,
   ...
   function Mean (Stats : in Stats_Type) return Real_Type is
   begin
   ...
   function Standard_Deviation (Stats : in Stats_Type) return Real_Type is
      Sigma : Real_Type;
   begin
      if Stats.Count > 1 then
         Sigma :=
           abs (Real_Type (Stats.Count) * Stats.Sum_Squared -
                Stats.Sum * Stats.Sum);
         Sigma :=
            Elementary.Sqrt
              (Sigma /
               (Real_Type (Stats.Count) * Real_Type (Stats.Count - 1)));
      else
         Sigma := 0.0;
      end if;
