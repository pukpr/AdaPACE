package Pace.Signals is
   ...
   protected type Shared_Data (Obj : access Msg'Class) is
      entry Read (Buffer : out Msg'Class);
      entry Write (Buffer : in Msg'Class);
   private
      Ready : Boolean := False;
   end Shared_Data;

   Data_Mismatch : exception;
