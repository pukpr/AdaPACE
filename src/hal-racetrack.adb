...
package body Hal.Racetrack is
   ...
   Straightaway_Length : Float :=
     Float (Num_Slots) * Slot_Distance / 2.0 - Ada.Numerics.Pi * Radius;
   ...
   begin
      ...
      for I in 1 .. Num_Slots loop
         Slots_On_Track (I) := Num_Intervals * (Index - 1) + 1;
         Index := Index + 1;
      end loop;
   ...
   begin
      ...
      while Current_Z > -1.0 * Straightaway_Length loop
         Track (I) := Slot_Position'
                        (Phi => Current_Phi, X => Track_Width, Z => Current_Z);
         Current_Z := Current_Z - Slot_Distance / Float (Num_Intervals);
         I := I + 1;
      end loop;
      -- reset to Straightaway_Length
      Arc_Length := -1.0 * Straightaway_Length - Current_Z;
      ...
      while Arc_Length < Half_Circle_Arc_Length loop
         ...
         Track (I) := Slot_Position'
                        (Phi => Current_Phi,
                         X => Track_Width * Cos
                                              ((Current_Phi - Pi / 2.0) / 2.0),
                         Z => Current_Z - Radius *
                                            Sin (Current_Phi - Pi / 2.0));

         Arc_Length := Arc_Length + Slot_Distance / Float (Num_Intervals);
         I := I + 1;
      end loop;

      Current_Z := Current_Z + Arc_Length - Half_Circle_Arc_Length;
      Current_Phi := -1.0 * Pi / 2.0;

      -- the long z-axis where X is 0 and Z increases to 0
      while Current_Z < 0.0 loop
         Track (I) := Slot_Position'
                        (Phi => Current_Phi, X => 0.0, Z => Current_Z);
         Current_Z := Current_Z + Slot_Distance / Float (Num_Intervals);
         I := I + 1;
      end loop;

      Arc_Length := Current_Z;
      Current_Z := 0.0;
