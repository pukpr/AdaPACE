...
package body Aunit.Test_Results is
   ...
   procedure Start_Test (R : in out Result; Subtest_Count : Natural) is
   begin
   ...
   procedure Add_Success (R : in out Result;
                          Test_Name, Routine_Name : String_Access) is
   begin
      Success_Lists.Extend (R.Successes_List,
                            Test_Success'(Test_Name, Routine_Name));
   end Add_Success;

   --  Record a test failure
   procedure Add_Failure (R : in out Result;
                          Test_Name, Routine_Name : String_Access;
                          E : Exception_Occurrence) is
   begin
      Failure_Lists.Extend (R.Failures_List,
                            Test_Failure'(Test_Name, Routine_Name,
                                          Save_Occurrence (E)));
   end Add_Failure;

   --  Record a test error
   procedure Add_Error (R : in out Result;
                        Test_Name, Routine_Name : String_Access;
                        E : Exception_Occurrence) is
   begin
      Failure_Lists.Extend (R.Errors_List,
                            Test_Failure'(Test_Name, Routine_Name,
                                          Save_Occurrence (E)));
   end Add_Error;
