...
package body Ses.Pp is
   ...
   begin
      if not Initialized_Parser then
         begin
            Dll.Open (Handle, "", 16#100#); -- RTLD_GLOBAL
         exception
      ...
      loop
         ...
         begin
            ...
            Text_IO.Put_Line (Ses.Output_Marker);
         exception
   ...
   exception
      when Text_IO.End_Error =>
         Ses.Os_Exit (0);
      when E : others =>
         Text_IO.Put_Line ("PPParser:" & Ada.Exceptions.Exception_Information(E));
   end Parser_Task;
