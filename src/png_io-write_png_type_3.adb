...
is
   F         : File_Type;
   L         : constant Positive range 1 .. 256                      :=
      Palette_Length (P);
   BD        : constant array (1 .. 256) of Depth range One .. Eight :=
     (1 .. 2    => One,
      ...
      5 .. 16   => Four,
      17 .. 256 => Eight);
   Bit_Depth : constant Depth range One .. Eight                     :=
      BD (L);
begin
   Start_File (Filename, X, Y, Three, Bit_Depth, Interlace, F);
   ...
   declare
      ...
      UDB : Buffer_Pointer        :=
         new Buffer (1 .. Image_Size (Three, Bit_Depth, X, Y, Interlace));
      ...
      --whole
      -- (non-interlaced) image. In this case, it is not used in the
      --procedure. The
      -- procedure cannot be called for a zero size sub-image, since its X and
      --Y
      ...
      procedure Write_Image (X, Y : in Dimension; Pass : Pass_Number := 1) is
         Bps      : constant Stream_Element_Count :=
            Bytes_per_Scanline (Three, Bit_Depth, X);
         Scanline : Stream_Element_Array (1 .. Bps);

         function Checked_Value (V : Sample) return Unsigned_8 is
         begin
            if Positive (V + 1) > L then
               Raise_Exception
                 (Call_Error'Identity,
                  "Pixel value greater than palette length.");
            else
               return Unsigned_8 (V);
            end if;
         end Checked_Value;
      ...
      begin
         ...
         for Row in  0 .. Coordinate (Y - 1) loop
            ...
            begin
                     ...
                     declare
                        Offset : constant array (Depth_1_2_4) of Coordinate :=
                          (7,
                           3,
                           1);
                        Shift  : constant array (Depth_1_2_4) of Natural    :=
                          (1,
                           2,
                           4);
               ...
               --Specification,
               -- the scanlines are not filtered for type 3 images. We simply
               --have to
