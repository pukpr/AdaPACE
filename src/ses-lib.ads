...
package Ses.Lib is
   ...
   function Argument (Name : in String; Default : in String) return String;
   function Argument (Name : in String) return Boolean;
   function Localhost return String;

   function Run (Index : Integer;
                 Target, Dir, Exec, Match : in String;
                 Display : in String := Localhost) return Pd;
   function Re_Pattern (Text : String) return Re;

   function Peek (Pid : Pd; Symbol : String; Timeout : Integer := -1)
                 return String;

   procedure Poke (Pid : in Pd;
                   Symbol : in String;
                   Value : in String;
                   Timeout : Integer := -1);
   ...
   function Drawers_Ready return Boolean; -- True if all Pid's up
   ...
   -- Utilities for parsing space delimited strings
   function Select_Field (Item : String; Field_No : Integer) return String;
