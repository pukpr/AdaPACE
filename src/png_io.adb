...
package body PNG_IO is
   ...
   -- data stream, supplied as the two parameters.
   -- (See the PNG Specification Version 1.2, section 5.)
   ...
   begin
      return (CMG and 2#1111#) = 8 and
             (Unsigned_16 (CMG) * 256 + Unsigned_16 (FLG)) mod 31 = 0;
   ...
   PNG_Signature : constant Stream_Element_Array :=
     (16#89#,
      16#50#,
      16#4E#,
      16#47#,
   ...
   package Chunk_Ordering is
      ...
      -- An encoding/LUT of the rules laid out in the PNG specification
      -- (actually the table is best defined in the 2nd draft standard
      -- ISO/IEC 15948, page 26). This should not be confused with the
      ...
      --function  After_PLTE(C : Unsigned_32) return Boolean; -- Not currently
      --used.
   ...
   Bit_Depth_Table   : constant array (Depth) of Stream_Element            :=
     (1,
      2,
      4,
      8,
      16);
   Colour_Type_Table : constant array (Colour_Type_Code) of Stream_Element :=
     (0,
      2,
      3,
      4,
   ...
   type PNG_File_Descriptor is record
      ...
      Text_Strings                                                     :
        Text_Item_Pointer;
      Number_of_Chunks                                                 :
        Natural := 0; -- Unrecognised ancillary chunks
      Ancillary_Chunks                                                 :
        Chunk_List;   -- are tacked on here as a list.
   ...
   begin
      return Positive'Max
               (1,
                (Bits_per_Pixel (Colour_Type, Bit_Depth)) / 8);
   end Bytes_per_Pixel;
   ...
   -- of bits.
   begin
      return Stream_Element_Count (Bits_per_Pixel (Colour_Type, Bit_Depth) *
                                   Scanline_Width +
                                   7) /
             8;
   ...
   is
      ...
      begin
         if W = 0 or H = 0 then
            ...
            return Stream_Element_Count (H) *
                   (Bytes_per_Scanline (Colour_Type, Bit_Depth, W) + 1);
         end if;
      end Sub_Image_Size;
   ...
   begin
      return F.Interlace = 1; -- Simplified 26 May 2002. The byte can only be
                              --0 or 1.
   end Interlaced;

   function Mean (X, Y : Stream_Element) return Stream_Element is
      -- Function to compute the mean value used in the Average
      -- filter described in the PNG Specification V1.0 Section 6.5.
   ...
   function PaethPredictor (A, B, C : Stream_Element) return Stream_Element is
      -- This code is based on the pseudocode given in the PNG Specification
      --V1.0,
      -- Section 6.6.
   ...
   begin
      if F = null then
         -- The file is not open or has not been opened (it might already have
         --been closed.)
         -- The standard packages such as Ada.Direct_IO raise Status_Error in
         --this situation
         -- so we do the same here. We cannot allow this to go undetected,
         --otherwise we will
         -- get exceptions in the following code when we attempt to access
         --elements of the
         -- record (not) accessed by F.
         Raise_Exception
           (Status_Error'Identity,
            "Attempt to close non-open file.");
      end if;
      ...
      declare
         function Index
           (R, C : Coordinate;
            ...
            return Stream_Element_Offset
         is
            -- Computes the index of the desired byte within the image or
            --sub-image.
         ...
         begin
            return 1 +
                   Stream_Element_Offset (R) *
                   Stream_Element_Offset (Bytes_per_Scanline (CT, BD, W) +
                                          1) +
                   1 +
                   (Stream_Element_Offset (C) * Stream_Element_Offset (Bpp)) /
                   8;
         end Index;
         pragma Inline (Index);
   ...
   -- Low-level functions to fetch the byte/word at a given coordinate in the
   --image.
   ...
   is
   begin
      return Unsigned_8 (F.Uncompressed_Data (Pixel_Index (F, R, C) +
                                              Offset));
   end U8;
   pragma Inline (U8);
   ...
   begin
      return Shift_Left (Unsigned_16 (D (P)), 8) or
             Unsigned_16 (D (P + 1));
   end U16;
   pragma Inline (U16);

   -- Low-level function to fetch bits, half nibbles or nibbles at a given
   --coordinate
   -- in the image. Since the leftmost pixels are in the high order bits of
   --the byte
   -- PNG spec, section 2.3), we have to reverse the position by subtraction
   --from 7/3/1.
   -- Modified 18 July 2000 to change BD from Depth_1_2_4 to Unsigned_8 for
   --efficiency.
   ...
   function U1_2_4
     (F    : PNG_File;
      ...
      return Unsigned_8
   is
      B : constant Unsigned_8                    := U8 (F, R, C); -- The byte
                                                                  --containing
                                                                  --the pixel.
      T : constant array (Boolean) of Coordinate :=
        (False => C,
         ...
         if I > F.Palette.Size then
            Raise_Exception
              (Format_Error'Identity,
               "Invalid pixel data in palette image.");
         else
            return I;
         end if;
      end Validate;
   ...
   begin
      ...
      end case;                                    -- reading this shouldn't
                                                   --happen.
   end Palette_Index;
   ...
   begin
      return Chunk'
        (1,
         To_Chunk_Name (sRGB),
         Stream_Element_Array'(0 => Rendering_Intent'Pos (R)),
         Before_PLTE);
   ...
   -- The gamma value used here is laid down in the PNG Specification V1.2,
   --p22.
   begin
      return Gamma_Chunk (45_455);
   ...
   begin
      if N <= F.Number_of_Texts then
         -- We work through the list here in reverse order. If N =
         --F.Text_Strings
         -- we return the first list item, if N = 1 we return the last. (The
         --list
   ...
   begin
      if N <= F.Number_of_Chunks then
         ...
         -- we return the first list item, if N = 1 we return the last. (The
         --list
         -- was added to at the head so the first item is the last chunk found
         --in
         -- the file.
   ...
   function "&" (Left : Chunk_List; Right : Chunk) return Chunk_List is
      T : Chunk_List := Left; -- This might be a null list!
   begin
      if T = null then

         -- The user was able to supply a null list on the left and has
         -- done so. We handle this without complaint and return a valid
         -- result:

         return To_Chunk_List (Right);
      end if;

      -- Otherwise, we need to feel our way down to the end of the list
      -- and stick a new element there with the value of Right in it.

      while T.Link /= null loop
         T := T.Link;
      end loop;
      T.Link := new Chunk_List_Element'(Right.Size, Right, null);
      return Left;
   end "&";
      ...
      if Known_Chunk (N) and then Position (N) /= Where then
         Raise_Exception
           (Argument_Error'Identity,
            ...
            Name &
            " should be " &
            Chunk_Position'Image (Position (N)));
      end if;

      return (Data'Length, Name, Data, Where);
   end Ancillary_Chunk;

   function Gamma_Chunk (Gamma : Natural := 100_000) return Chunk is
   ...
   begin
      ...
      -- Check the keyword for validity. (PNG Specification 1.2 section 4.2.3).

      if Keyword'Length = 1                  -- There must be at least one
                                             --character.
        or else Keyword (Keyword'First) = Space -- Leading spaces are not
                                                --permitted.
      ...
      return Chunk'
        (L,
         To_Chunk_Name (PNG_IO.tEXt),  -- The chunk code, not the parameter
                                       --Text!
         To_Stream_Element_Array (Keyword & NUL & Text),
         Anywhere);
   end Text_Chunk;

   function Chromaticity_Chunk
     (White_Point, Red_Primary, Green_Primary, Blue_Primary : Pair)
      return                                                  Chunk
   is
   ...
   function Physical_Chunk (Value : Pair; Metre : Boolean) return Chunk is
      D : constant Stream_Element_Array :=
         To_Buffer_4 (Unsigned_32 (Value.X)) &
   ...
   -- Procedures for writing PNG files. There are variants of the user visible
   --generics
   -- for each type of PNG file and for 8-bit and 16-bit sample sizes.
   -- However, most of the actual code for file output is common.

   -- The procedure Write_Chunk is responsible for computing the CRC of the
   --chunk, as
   -- well as the length.
   ...
   -- as a constant array on the stack.
   ...
   procedure Write_IDAT_Chunk
     (F   : in File_Type;
      UDB : in out Buffer_Pointer)
   is

      -- Takes a buffer of uncompressed pixel data after filtering, if any,
      -- compresses it, and outputs one or more IDAT chunks according to the
      -- buffers of data passed back by Zlib.
      -- It then deallocates the uncompressed data buffer.

      Buffer_Size : constant := 8 * 1024; -- The size of the buffers is
                                          --somewhat
                                          -- arbitrary. Using a larger buffer
                                          --means
                                          -- fewer calls back and forth, and
                                          --fewer
                                          -- IDAT chunks in the output file.

      -- We pass data to Zlib in bufferfuls, and therefore we have to keep
      --track
      -- of where we are in the uncompressed data buffer, UDB. This is the
      --purpose
      -- of P, which is a pointer to the element of UDB which will be the first
      -- byte of the next bufferful to be passed into Zlib.

      P : Stream_Element_Offset := UDB'First;
      ...
      is
         ...
         -- Pass a full buffer of data to Zlib, unless there is less than a
         --full buffer
         -- left in the uncompressed data buffer, in which case, pass what
         --there is left.
         -- We have to be careful here not to mix the index values used with
         --Data, which
         -- are determined by Zlib, and those used with UDB, which are
         --determined here.

         -- P and Q are the first and last index values into UDB, L is the
         --length of the
         -- slice of UDB to be passed to Zlib.

         Q : constant Stream_Element_Offset :=
            Stream_Element_Offset'Min (P + Buffer_Size - 1, UDB'Last);
         L : constant Stream_Element_Count  := Q - P + 1;
      begin
         Last := Data'First + L - 1;

         -- Note that we have to assign to a slice of Data here, because when
         --we pass the
         -- last bufferful, it may be smaller than the length of Data. Most of
         --the time
         -- the slice will be the whole of Data.

         Data (Data'First .. Last) := UDB (P .. Q);

         P := Q + 1; -- Leave P pointing to the first byte of the next
                     --bufferful.
      end Data_In;

      B1, B2             : Stream_Element;   -- To store the first two bytes
                                             --of the Zlib stream.
      B1_Valid, B2_Valid : Boolean := False;

      procedure Data_Out (Data : in Stream_Element_Array) is
      -- Output an IDAT chunk with whatever is passed to us from the
      --compressor.
      -- Note that this could result in small IDAT chunks, particularly at the
      -- start or end of the compressed data, but there is no easy way to avoid
      -- this. An IDAT chunk of length 1 (or even 0) is legal, in any case, so
      -- it will not cause a problem. The first two bytes passed are stored in
      -- two variables and later checked for conformance to the PNG
      --specification
      -- (the default Zlib parameters should ensure this, but it is as well to
      --be
      -- sure in case Zlib_Ada or Zlib is changed in the future).
      begin

         if Data'Length = 0 then
            return;
         end if;  -- This is included because Zlib_Ada
                  -- V1.2 will pass zero length buffers.
                  -- This statement can be removed when
                  -- Zlib_Ada no longer does this, but
                  -- should probably then be replaced
                  -- with an assertion.
         if not B1_Valid then
            B1       := Data (Data'First);   -- This must be the first block
                                             --of compressed data.
            B1_Valid := True;
            if Data'Length > 1 then -- There is at least one more byte in the
                                    --data array.
               B2       := Data (Data'First + 1);
               B2_Valid := True;
            end if;
         end if;

         -- If the first two bytes of the compressed data are passed in
         --separate calls
         -- to this routine, we need to deal with the second byte on the
         --second call,
         -- in which case it must be the first byte of the data array. This is
         --unlikely
         -- but we can't be sure Zlib will not work like this (currently it
         --passes the
         -- first two bytes together, but without any other data, presumably
         --to get the
         -- header information out of the way before compressed data is
         --output).

         if not B2_Valid then
            B2       := Data (Data'First);
            B2_Valid := True;
         end if;

         Write_Chunk (F, IDAT, Data);

      end Data_Out;

      use ZLib;

      procedure Translate is new Generic_Translate (Data_In, Data_Out);

      Z : Filter_Type;

   begin
      Deflate_Init (Z);
      Translate (Z, Buffer_Size, Buffer_Size);
      Close (Z);
      Deallocate (UDB);
      pragma Assert (Valid_Zlib_Header (B1, B2));
   end Write_IDAT_Chunk;
   ...
   pragma Inline (Finish_File);

   generic
      Bpp : in Stream_Element_Offset := 1;   -- The offset between
                                             --corresponding bytes
                                             -- of adjacent pixels within a
                                             --scanline, as
   ...
   procedure Append_to_Buffer
     (B : in Buffer_Pointer;
      P : in out Stream_Element_Offset; -- The next free element in B.
      S : in Stream_Element_Array)
   is
      L : constant Stream_Element_Offset := P + S'Length - 1;
      pragma Assert (L <= B'Last);
   begin
      B.all (P .. L) := S;
      P              := L + 1;
...
begin
   ...
   if To_Chunk_Name (IHDR) /= "IHDR" or
      To_Chunk_Name (PLTE) /= "PLTE" or
      To_Chunk_Name (IDAT) /= "IDAT" or
      To_Chunk_Name (IEND) /= "IEND" or
      To_Chunk_Name (cHRM) /= "cHRM" or
      To_Chunk_Name (gAMA) /= "gAMA" or
      To_Chunk_Name (sRGB) /= "sRGB" or
      To_Chunk_Name (pHYs) /= "pHYs" or
      To_Chunk_Name (tEXt) /= "tEXt" or
      To_Chunk_Name (zTXt) /= "zTXt" or
      To_Chunk_Name (tRNS) /= "tRNS" or
      To_Chunk_Name (iCCP) /= "iCCP" or
      To_Chunk_Name (iTXt) /= "iTXt" or
      To_Chunk_Name (bKGD) /= "bKGD" or
      To_Chunk_Name (sBIT) /= "sBIT" or
      To_Chunk_Name (sPLT) /= "sPLT" or
      To_Chunk_Name (hIST) /= "hIST" or
      To_Chunk_Name (tIME) /= "tIME"
   then
      Raise_Exception
