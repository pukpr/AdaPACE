...
begin
   ...
   declare
      ...
      ESL : constant Float := Float (Encoded_Sample'Last);
      SL  : constant Float := Float (Linear_Sample'Last);

   begin

      for I in  Encoding_LUT'Range loop
         Encoding_LUT (I) :=
           Encoded_Sample (ESL * Non_Linear (Float (I) / SL));
      end loop;

      for I in  Decoding_LUT'Range loop
         Decoding_LUT (I) := Linear_Sample (SL * Linear (Float (I) / ESL));
      end loop;
