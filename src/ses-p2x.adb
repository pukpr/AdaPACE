...
package body Ses.P2X is
   ...
   procedure Process_Command (Str : String) is
      ...
      P4_Marker : constant String := Ses.Output_Marker; -- "__ p4 __";
...
begin
   Save_Action (Set_Display'(Pace.Msg with Set => Default));
   Save_Action (Push'(Pace.Msg with Set => Default));
   Save_Action (Press'(Pace.Msg with Set => Default));
   Save_Action (Release'(Pace.Msg with Set => Default));
   Save_Action (Place'(Pace.Msg with Set => Default));
   Save_Action (Move'(Pace.Msg with Set => Default));
   Save_Action (Keyboard'(Pace.Msg with Set => Default));
   Save_Action (Quit'(Pace.Msg with Set => Default));
   ...
   if Run_A2x then
      ...
      Pace.Log.Put_Line ("Note: P2X disabled, PACE_A2X=0 (no spawned A2X process)");
   end if;
